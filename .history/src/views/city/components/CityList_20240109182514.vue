...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(
      () => props.letter,
      (newLetter) => {
        if (newLetter && scroll.value && wrapperRef.value) {
          // Example: Assuming you have an id or class on the element that can be selected
          const element = wrapperRef.value.querySelector(`#${newLetter}`) // or .${newLetter} for class
          if (element) {
            // scrollToElement(element, time, offsetX, offsetY, easing)
            scroll.value.scrollToElement(element as HTMLElement, 500, true, true, undefined)
          }
        }
      }
    )
